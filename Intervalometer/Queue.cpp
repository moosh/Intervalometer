...
int Queue::scheduleFunction(queuedFunction func, const char * id, unsigned long initialRun, unsigned long recur, int message, void* ctx)
{
	int rv = 0;
...
int Queue::scheduleChangeFunction(const char * id, unsigned long nextRunTime, unsigned long newRecur, int message, void* ctx)
{
	queueItem target;
    int rv = -1;
    for (int i = 0; i < _itemsInQueue; ++i)
...
int Queue::Run(unsigned long now)
{
    ...
    for (int i = 0; i < _itemsInQueue; ++i)
    {
        if(_queueGetTop(target)==0)
        {
            ...
            {
                int tRv;
                tRv = (target.fPtr)(now, target.message, target.ctx);
                if(tRv == 0)
                {
                    rv++;
                }
